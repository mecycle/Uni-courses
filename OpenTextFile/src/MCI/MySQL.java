...
public class MySQL {
	public ArrayList<String> connect_sql(String input,int match) {
	    ...
	    try {
	      ...
	      ResultSet rs = null;
	      rs = stmt.executeQuery("select * from test.`abc` WHERE Body LIKE '%"+input+"%' ");
	      ...
	      for(String s: user) {
	    	  System.out.println("the problem ID is  " +s);
	      }
	      for(String s: user) {	    	 
	    	  rs = stmt.executeQuery("select * from test.`abc` WHERE parentId ="+ s +" AND PostTypeId = 2");
	    	  while (rs.next()) {
